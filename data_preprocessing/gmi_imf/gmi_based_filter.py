import pandas as pd
import numpy as np
#def gmi_based_filter_slow(rad, output_table, stm=None, etm=None, ftype="fitacf", coords="mlt",
...
def gmi_based_filter(rad, output_table, stm=None, etm=None, ftype="fitacf", coords="mlt",
    ...
    if isKp_based:
        tbl_nm = "kp"
        # Fetch the data between stm and etm
        if (stm is not None) and (etm is not None):
	    command = "SELECT datetime, kp FROM {tb} WHERE (datetime BETWEEN '{stm}' AND '{etm}') " +\
		      "AND (kp BETWEEN {kp_min} AND {kp_max})"
	    command = command.format(tb=tbl_nm, stm=stm, etm=etm,
				     kp_min=kp_lim[0],kp_max=kp_lim[1])

        # Fetch all the data if any of them is None
	else:
	    command = "SELECT datetime, kp FROM {tb} WHERE kp BETWEEN {kp_min} AND {kp_max}"
	    command = command.format(tb=tbl_nm, kp_min=kp_lim[0],kp_max=kp_lim[1])
        cur_gmi.execute(command)
        kp_columns = [x[0] for x in cur_gmi.description]
        kp_dat = cur_gmi.fetchall()

        # Load kp data to a DF
        print("Loading Kp data...")
        df_kp = pd.DataFrame(data=kp_dat, columns=kp_columns)

        # Sample kp data at each 10 minutes 
        df_kp = resample_kp(df_kp)

    # close conn_gmi opened for gmi db
    conn_gmi.close()

    # Load iscat data to a db
    print("Loading iscat data...")
    command = "SELECT * FROM {tb} WHERE datetime BETWEEN '{stm}' " +\
              "AND '{etm}'"
    command = command.format(tb=input_table, stm=stm, etm=etm)
    try:
        cur_iscat.execute(command)
        iscat_columns = [x[0] for x in cur_iscat.description]
        iscat_dat = cur_iscat.fetchall()
        df_iscat = pd.DataFrame(data=iscat_dat, columns=iscat_columns)
    except Exception, e:
        logging.error(e, exc_info=True)

    # Join the two tables
    print("Joining the two tables...")
    df_iscat = df_iscat.join(df_kp, on="datetime", how="left")

    # Drop rows whose Kp values are NaNs
    df_iscat = df_iscat.loc[~np.isnan(df_iscat.kp.values), :] 
    ...
    for i, rw in df_iscat.iterrows():

        # insert the data into a table
        vel, lat, lt, azm, dtm =\
        rw["vel"], rw["mag_glatc"], rw["mag_gltc"], rw["mag_gazmc"], rw["datetime"]
...
def resample_kp(df_kp):
    """Resamples Kp data at every 10 mintues in the form of xx:x5"""
    kp_values = df_kp.kp.values
    dt_values = df_kp.datetime.values
    dfs_kp = []
    for m in range(5, 60*3, 10):
        dt_m = dt_values + np.timedelta64(m, 'm')
        df_m = pd.DataFrame(data={"kp":kp_values}, index=dt_m)
        df_m.index.name = "datetime"
        dfs_kp.append(df_m)

    df_kp = pd.concat(dfs_kp).sort_index()
    return df_kp
...
def main(run_in_parallel=False):
    ...
    output_table = "test_kp_" + kp_text + "_" + ftype
    isKp_based=True
    isSymH_based=False
    symh_min=-50
    ten_min_median_dbname=None
